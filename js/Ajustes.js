  ...
  divisor.forEach(function(i) {
    ...
    for(a=0; a<=tamanio; a++){
      ...
      if( addwords [ a ] == "/></Fields><Tables><Table name=" || addwords [ a ] == "/></Columns></Table><Table name=" || addwords[ a ] == "><Tables><Table name="){
        ...
        $("#cuadros").append('<p id="campotl' + addwords [ a + 1 ] + '" class="prueba" style="margin-top: 20px; text-transform: uppercase;">Dirección: ' + addwords [ a + 3 ] + ' </p> '+'<textarea class="form-control noresize tamaño10px pruebax" id="campo'+ addwords[a+1] +'" rows="15" readonly></textarea>');
...
function salidadouble(variable){
  ...
  $("#salida").append('<p class="hola"> public double "'+variable+'" {get; set;} </p>')
}
